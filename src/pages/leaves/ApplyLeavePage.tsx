...
const ApplyLeavePage: React.FC = () => {
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm<CreateLeaveRequestData>();
  ...
  const { data: holidaysData } = useQuery({
    ...
    queryFn: () => getHolidays({ year: new Date().getFullYear(), isActive: true }),
    onError: (err: any) => setError(getErrorMessage(err)),
  });
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-gray-900 mb-6">Apply for Leave</h1>
          ...
          {startDate && endDate && (
            <div className="bg-gray-50 p-4 rounded-md">
              <p className="text-sm text-gray-700">
                <span className="font-medium">Duration:</span> {duration} working day(s)
              </p>
            </div>
          )}
